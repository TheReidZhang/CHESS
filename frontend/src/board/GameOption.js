...
function GameOption(props) {
  ...
  const onToggle = (isOpen, e, metadata) => {
    if (metadata["source"] === "click") setShow(!show);
  };
    ...
    <Dropdown show={show} drop="left" onToggle={onToggle}>
      ...
      <Dropdown.Menu style={{ minWidth: "auto" }}>
        <Dropdown.Item onClick={props.takeback}>
          <div style={{ textAlign: "center", fontSize: "75%" }}>Takeback</div>
        </Dropdown.Item>
        <Dropdown.Item>
          <div style={{ textAlign: "center", fontSize: "75%" }}>Resign</div>
        </Dropdown.Item>
        <Dropdown.Item>
          <div style={{ textAlign: "center", fontSize: "75%" }}>Draw</div>
        </Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown>
  );
